...
export async function getUsers(
  ...
  try {
    ...
    console.log(err);
    res.writeHead(CODES.INTERNET_SERVER_ERROR, {
      'Content-Type': 'application/JSON',
    });
    res.end(
      JSON.stringify({ message: 'Ups! something was wrong on server...' })
    );
  }
}
  ...
  try {
    ...
    req.on('end', async () => {
      try {
        ...
        if (!isUserCreate(reqBody)) {
          res.writeHead(CODES.BAD_REQUEST, {
            'Content-Type': 'application/JSON',
          });
          res.end(
        ...
        res.end(
          JSON.stringify({ message: 'Ups! May you write incorrect JSON' })
        );
      }
    });
  } catch (err) {
